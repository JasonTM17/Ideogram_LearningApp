...
create or replace function private.evaluate_and_submit_activity_attempt(
  p_user_id uuid,
...
declare
  ...
  vocabulary_entry_count integer;
  vocabulary_entry_position integer;
...
begin
  ...
  if activity_record.activity_type = 'vocabulary'
    and persisted_receipt.completion_state = 'completed' then
    vocabulary_entry_count := jsonb_array_length(activity_record.payload -> 'entries');
    if vocabulary_entry_count < 1 then
      raise exception using
        errcode = '22023',
        message = 'Vocabulary review source is unavailable.';
    end if;

    for vocabulary_entry_position in 1..vocabulary_entry_count loop
      perform private.initialize_review_item(
        p_user_id,
        p_content_release_id,
        p_activity_id,
        format('vocabulary-%s', vocabulary_entry_position)
      );
    end loop;
  end if;
