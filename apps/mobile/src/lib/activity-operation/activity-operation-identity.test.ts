...
describe('activity operation identity store', () => {
  it('rejects storage keys that SecureStore cannot accept', () => {
    expect(
      () =>
  ...
  it('clears a retained identity before starting a fresh installation stream', async () => {
    const { storage, values } = createStorage();
    ...
    const store = createStore(storage, () => newDeviceId, storageKey);
    ...
    await expect(store.reserve()).resolves.toEqual({
      deviceId: newDeviceId,
      deviceSequence: 2,
    });
  });
