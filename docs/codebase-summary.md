...
- Web offline sync now has IndexedDB storage plus a service-worker background sync path in source. Native offline sync now has SecureStore storage plus Expo BackgroundTask registration in source. Both adapters keep queued writes namespaced by user/session, but real-device/browser/deployed-worker proof is still pending.
