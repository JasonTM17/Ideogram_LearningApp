...
| Read completed AI tutor replay            | `private.read_ai_tutor_turn_replay()` via `POST /api/v1/ai/tutor/turn`            | Active learner and exact conversation/turn/request hash match                | Runs before new-turn policy checks; returns only a completed stored receipt                       |
| Begin AI tutor turn                       | `private.begin_ai_tutor_turn_v2()` via `POST /api/v1/ai/tutor/turn`               | Active learner, current provider consent, active language pack, atomic quota | Recomputes canonical hash; binds user/conversation/turn/hash and lease token before provider call |
| Complete AI tutor turn                    | `private.complete_ai_tutor_turn_v2()`                                             | Same learner/hash, unexpired lease token, pending state                      | Stores structured response, token usage, model/config snapshot, and cost estimate                 |
| Fail AI tutor turn                        | `private.fail_ai_tutor_turn_v2()`                                                 | Same learner/hash, unexpired lease token, pending state                      | Releases reservation; stores normalized failure code only                                         |
