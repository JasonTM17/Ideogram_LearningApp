...
- These Postgres helpers are the persistence layer. The catalog read route
  exists, and `private.submit_review_event` is now surfaced through
  `POST /api/v1/learning/reviews/submit`; the other learning mutation routes are
...
| POST   | `/api/v1/learning/activities/submit`    | Activity attempt submission  | Planned contract only |
...
The learning catalog is now implemented. `POST /api/v1/learning/reviews/submit`
is live. Activity submission, the other learning mutation routes, and full
...
- Review submission coverage currently includes route, repository, and api-client
  tests, web/api-client typecheck, pgTAP 42/42, existing-login provisioning
  drift probes, the dedicated lock-order integration test, and the full
  workspace format/lint/typecheck/test/build/audit gates. GitHub Actions also
  defines a local-Supabase database job; confirm its hosted run separately.
