...
| Browser IndexedDB sync queue / Cache Storage media                                                                                                                                                                                                                                                                         | `(userId, sessionEpoch)` / authenticated user | Bounded local cache until receipt, discard, eviction, account switch, or local sign-out     | Not exported                            | Cleared on namespace mismatch or local sign-out                          | Queue stores validated mutation bodies; media stores only checksum-verified approved assets                                     |
| Expo SecureStore sync queue / document media cache                                                                                                                                                                                                                                                                         | `(userId, sessionEpoch)` / authenticated user | Bounded local cache until receipt, discard, eviction, account switch, or confirmed sign-out | Not exported                            | Cleared on namespace mismatch, reinstall sentinel, or confirmed sign-out | Background scheduling is best-effort and never proof of completion                                                              |

## Current constraints

- Signed URL revocation and provider-side deletion verification are not yet
  implemented.
- Queue namespace mismatch and installation-sentinel checks prevent known
  cross-account/reinstall replay; real-device background execution evidence is
